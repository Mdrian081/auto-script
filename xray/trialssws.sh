...
shadowsockslink2="ss://${shadowsocks_base64e}@${domain}:$ntls?path=ss-ws&security=tls&host=${domain}&type=ws#${user}"
shadowsockslink1="ss://${shadowsocks_base64e}@${domain}:$tls?mode=gun&security=tls&type=grpc&serviceName=ss-grpc&sni=${BUG_HOST}#${user}"
...
echo -e "Wildcard       : ${BUG_HOST}.${domain}"
echo -e "Port TLS       : ${tls}"
